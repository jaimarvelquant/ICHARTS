...
def get_available_dates():
    """
    Get list of available dates from nifty_future table only
    """
    host = "106.51.63.60"
    user = "mahesh"
    password = "mahesh_123"
    ...
    try:
        ...
        # Get unique dates from future table only
        query = """
        SELECT DISTINCT date FROM nifty_future
        ORDER BY date DESC
        LIMIT 100
        """
...
def get_ohlc_data_for_date(date):
    """
    Get OHLC data for a specific date from nifty_future table only
    """
    host = "106.51.63.60"
    user = "mahesh"
    password = "mahesh_123"
    ...
    try:
        ...
        # Get data only from nifty_future table
        query = """
        SELECT * FROM nifty_future 
        WHERE date = %s
        ORDER BY time
        """
...
def create_candlestick_chart_base64(df, date, interval_minutes=1):
    """
    Create a professional candlestick chart for future data with dark theme and return as base64 string
    """
    if df is None or len(df) == 0:
        return None
    ...
    # Plot candlesticks with future-focused styling
    for i, (idx, row) in enumerate(df_scaled.iterrows()):
        ...
        # Future data color scheme
        if close_price >= open_price:
            color = '#00ccff'  # Blue for bullish future
            edge_color = '#0099cc'
            wick_color = '#00ccff'
        else:
            color = '#ff8800'  # Orange for bearish future
            edge_color = '#cc6600'
            wick_color = '#ff8800'
        ...
        if body_height > 0:
            ...
            if close_price > open_price:
                ...
                ax.bar(i, body_height * 0.3, bottom=body_bottom + body_height * 0.7, 
                       color='#66ddff', edgecolor='none', width=0.7, alpha=0.6)
            else:
                # Bearish - add lighter bottom
                ax.bar(i, body_height * 0.3, bottom=body_bottom, 
                       color='#ffaa66', edgecolor='none', width=0.7, alpha=0.6)
        else:
    ...
    if interval_minutes == 1:
        title = f'NIFTY 50 Index • OHLC Candlestick Chart ({date}) - Future Data'
    else:
        title = f'NIFTY 50 Index • {interval_minutes}-Minute OHLC Candlestick Chart ({date}) - Future Data'
...
{price_change:+.2f} ({price_change_pct:+.2f}%)"""
    ...
    # Add legend for future data
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='#00ccff', edgecolor='#0099cc', label='Future Data')
    ]
...
def create_summary_chart_base64(df, date):
    """
    Create a professional summary chart for future data with dark theme and return as base64 string
    """
    if df is None or len(df) == 0:
        return None
    ...
    # Plot 1: Price movement with future-focused styling
    ax1.plot(df_scaled.index, df_scaled['close'], label='Future Close Price', 
            linewidth=3, color='#00ccff', alpha=0.9)
    
    ax1.fill_between(df_scaled.index, df_scaled['low'], df_scaled['high'], alpha=0.2, color='#00ccff', label='High-Low Range')
    ...
    title = f'NIFTY 50 Index • Daily Price Movement ({date}) - Future Data'
    ...
    colors = ['#00ff88', '#00ccff', '#ffaa00', '#ff4444']  # Future-focused colors
    
    bars = ax2.bar(categories, values, color=colors, alpha=0.8, edgecolor='#444444', linewidth=1.5)
    ax2.set_title('Daily OHLC Summary - Future Data', fontsize=16, fontweight='bold', color='#ffffff', pad=20)
...
def generate_chart():
    """Generate chart based on selected date, chart type, and timeframe for future data only"""
    ...
    if chart_type == 'candlestick':
        ...
        if interval_minutes == 1:
            chart_title = f'Candlestick Chart - {date} (Future Data)'
        else:
            chart_title = f'{interval_minutes}-Minute Candlestick Chart - {date} (Future Data)'
        ...
        chart_title = f'Summary Chart - {date} (Future Data)'
