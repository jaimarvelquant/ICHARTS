...
def get_available_dates():
    """
    Get list of available dates from database for NIFTY call options
    """
    host = "106.51.63.60"
    user = "mahesh"
    password = "mahesh_123"
    ...
    try:
        ...
        # Get unique dates from nifty_call table (adjust table name as needed)
        query = """
        SELECT DISTINCT date FROM nifty_call 
        ORDER BY date DESC
        LIMIT 100
        """
...
def get_available_strikes(date):
    ...
    try:
        ...
        query = """
        SELECT DISTINCT strike FROM nifty_call 
        WHERE date = %s
        ORDER BY strike
        """
...
def get_available_expiries(date, strike):
    ...
    try:
        ...
        query = """
        SELECT DISTINCT expiry FROM nifty_call 
...
def get_call_data_for_date_strike_expiry(date, strike, expiry):
    """
    Get NIFTY call option data for a specific date, strike price, and expiry date
    """
    host = "106.51.63.60"
    user = "mahesh"
    password = "mahesh_123"
...
def create_call_candlestick_chart(df, date, strike, expiry, interval_minutes=1):
    """
    Create a professional candlestick chart for NIFTY call options
    """
    if df is None or len(df) == 0:
        return None
    ...
    if interval_minutes == 1:
        title = f'NIFTY Call Option • Strike: {strike} • Expiry: {expiry} • OHLC Chart ({date})'
    else:
        title = f'NIFTY Call Option • Strike: {strike} • Expiry: {expiry} • {interval_minutes}-Minute OHLC Chart ({date})'
...
@app.route('/')
def index():
    """Main page with date and strike selection"""
    dates = get_available_dates()
    return render_template('nifty_call_index.html', dates=dates)
...
@app.route('/generate_call_chart', methods=['POST'])
def generate_call_chart():
    """Generate call option chart based on selected date, strike, expiry, and timeframe"""
    ...
    df = get_call_data_for_date_strike_expiry(date, strike_float, expiry)
    if df is None:
    ...
    chart_base64 = create_call_candlestick_chart(df_resampled, date, strike_float, expiry, interval_minutes)
    if chart_base64 is None:
        return jsonify({'error': 'Failed to generate call option chart'})
        
    if interval_minutes == 1:
        chart_title = f'NIFTY Call Option - Strike: {strike} - Expiry: {expiry} - {date}'
    else:
        chart_title = f'NIFTY Call Option - Strike: {strike} - Expiry: {expiry} - {interval_minutes}-Minute - {date}'
