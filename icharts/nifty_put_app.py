...
def create_put_candlestick_chart(df, date, strike, expiry, interval_minutes=1):
    """
    ...
    if interval_minutes == 1:
        title = f'NIFTY Put Option • Strike: {strike} • Expiry: {expiry} • OHLC Chart ({date})'
    else:
        title = f'NIFTY Put Option • Strike: {strike} • Expiry: {expiry} • {interval_minutes}-Minute OHLC Chart ({date})'
...
def generate_put_chart():
    """Generate put option chart based on selected date, strike, expiry, and timeframe"""
    ...
    chart_base64 = create_put_candlestick_chart(df_resampled, date, strike_float, expiry, interval_minutes)
    ...
    if interval_minutes == 1:
        ...
        chart_title = f'NIFTY Put Option - Strike: {strike} - Expiry: {expiry} - {interval_minutes}-Minute - {date}'
